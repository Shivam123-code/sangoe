...
export default function PlatformPage() {
  ...
  return (
    <div style={{ paddingTop: '0', minHeight: '100vh', background: 'var(--theme-bg)' }}>
      {/* Hero */}
        ...
        <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '100px 40px 80px', display: 'flex', alignItems: 'center', gap: '80px', position: 'relative', zIndex: 1 }}>
          ...
          <motion.div style={{ flex: '1', minWidth: 0, position: 'relative', flexShrink: 0, maxWidth: '560px' }} initial={{ opacity: 0, x: 60 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.9, ease: [0.16, 1, 0.3, 1], delay: 0.2 }}>
            ...
            <motion.div animate={{ y: [0, -8, 0] }} transition={{ repeat: Infinity, duration: 3, ease: 'easeInOut' }} style={{ position: 'absolute', top: '-18px', right: '-18px', background: 'var(--theme-card-bg)', border: '1px solid var(--theme-card-border)', borderRadius: '14px', padding: '10px 16px', boxShadow: '0 16px 40px rgba(0,0,0,0.35)', zIndex: 10, display: 'flex', alignItems: 'center', gap: '8px' }}>
              ...
              <span style={{ fontSize: '0.78rem', fontWeight: 700, color: 'var(--theme-text-main)', whiteSpace: 'nowrap' }}>FounderOS™ Live</span>
            </motion.div>
      ...
      <section style={{ padding: '60px 20px', maxWidth: '1100px', margin: '0 auto' }}>
        ...
        >
          ...
          <div>
            ...
            <h2 style={{ fontSize: '2rem', fontWeight: 900, color: 'var(--theme-text-main)', marginBottom: '12px' }}>{currentTab.title}</h2>
            <p style={{ color: 'var(--theme-text-sub)', fontSize: '1.05rem', lineHeight: 1.6, marginBottom: '24px' }}>{currentTab.subtitle}</p>
            <ul style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
              {currentTab.bullets.map((b, i) => (
                <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '10px', fontSize: '0.95rem', color: 'var(--theme-text-main)', fontWeight: 550 }}>
          ...
          <div style={{ background: 'var(--theme-bg)', border: '1px solid var(--theme-border)', borderRadius: '20px', height: '320px', padding: '24px', display: 'flex', flexDirection: 'column', justifyContent: 'center', position: 'relative', overflow: 'hidden' }}>
            ...
            {currentTab.id === 'founderos' && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                <div style={{ background: 'var(--theme-card-bg)', padding: '16px', borderRadius: '12px', border: '1px solid var(--theme-border)', boxShadow: '0 4px 12px rgba(0,0,0,0.01)' }}>
                  <div style={{ fontSize: '0.75rem', color: 'var(--theme-text-sub)', textTransform: 'uppercase', fontWeight: 700 }}>Founder Focus Matrix</div>
                  <div style={{ height: '8px', background: 'var(--theme-bg)', borderRadius: '4px', marginTop: '10px', overflow: 'hidden' }}>
                    <div style={{ height: '100%', width: '78%', background: '#7C3AED' }} />
                  </div>
                  <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.7rem', color: 'var(--theme-text-sub)', marginTop: '6px' }}>
                    <span>Delegated Tasks: 78%</span>
                    <span>Action Needed: 22%</span>
                  </div>
                </div>
                <div className="responsive-grid-2" style={{ gap: '16px' }}>
                  <div style={{ background: 'var(--theme-card-bg)', padding: '16px', borderRadius: '12px', border: '1px solid var(--theme-border)', boxShadow: '0 4px 12px rgba(0,0,0,0.01)', textAlign: 'center' }}>
                    <div style={{ fontSize: '1.5rem', fontWeight: 800, color: '#10b981' }}>98.2%</div>
                    <div style={{ fontSize: '0.7rem', color: 'var(--theme-text-sub)' }}>Compliance Rating</div>
                  </div>
                  <div style={{ background: 'var(--theme-card-bg)', padding: '16px', borderRadius: '12px', border: '1px solid var(--theme-border)', boxShadow: '0 4px 12px rgba(0,0,0,0.01)', textAlign: 'center' }}>
                    <div style={{ fontSize: '1.5rem', fontWeight: 800, color: '#3b82f6' }}>₹4.2M</div>
                    <div style={{ fontSize: '0.7rem', color: 'var(--theme-text-sub)' }}>Receivables Due</div>
                  </div>
                </div>
              </div>
            )}
            ...
            {currentTab.id === 'businessos' && (
              <div className="responsive-grid-3" style={{ gap: '12px' }}>
                {['Sales', 'HR', 'Finance', 'Projects', 'Assets', 'Compliance', 'BI', 'CS', 'SaaS'].map((cloud, i) => (
                  <div key={i} style={{ background: 'var(--theme-card-bg)', padding: '12px', borderRadius: '10px', textAlign: 'center', border: '1px solid var(--theme-border)', boxShadow: '0 2px 6px rgba(0,0,0,0.01)' }}>
                    <Cloud size={16} style={{ color: currentTab.color, marginBottom: '6px' }} />
                    <div style={{ fontSize: '0.7rem', fontWeight: 700, color: 'var(--theme-text-main)' }}>{cloud}</div>
                  </div>
                ))}
              </div>
            )}
            ...
            {currentTab.id === 'copilot' && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                <div style={{ background: 'var(--theme-card-bg)', padding: '12px 16px', borderRadius: '12px 12px 12px 0', maxWidth: '85%', alignSelf: 'flex-start', border: '1px solid var(--theme-border)', fontSize: '0.8rem', color: 'var(--theme-text-main)' }}>
                  What compliances are due for our manufacturing plant next week?
                </div>
                <div style={{ background: 'rgba(59, 130, 246, 0.1)', padding: '12px 16px', borderRadius: '12px 12px 0 12px', maxWidth: '85%', alignSelf: 'flex-end', fontSize: '0.8rem', color: 'var(--theme-text-main)', border: '1px solid rgba(59, 130, 246, 0.2)', display: 'flex', gap: '8px', alignItems: 'center' }}>
      ...
      <section style={{ background: 'var(--theme-card-bg)', padding: '80px 20px', borderTop: '1px solid var(--theme-border)' }}>
        <div className="responsive-grid-2" style={{ maxWidth: '1000px', margin: '0 auto', gap: '60px' }}>
          <div>
            <div style={{ display: 'inline-flex', padding: '8px', borderRadius: '8px', background: 'rgba(59, 130, 246, 0.1)', color: '#3B82F6', marginBottom: '12px' }}>
              <FileCode size={20} />
            </div>
            <h3 style={{ fontSize: '1.6rem', fontWeight: 800, color: 'var(--theme-text-main)', marginBottom: '12px' }}>Seamless API Integrations</h3>
            <p style={{ color: 'var(--theme-text-sub)', lineHeight: 1.6, fontSize: '0.95rem', marginBottom: '18px' }}>
            ...
            <span style={{ display: 'inline-flex', gap: '8px', flexWrap: 'wrap' }}>
              {['WhatsApp API', 'Razorpay', 'GST Portal', 'Tally Sync', 'Google Workspace'].map(t => (
                <span key={t} style={{ fontSize: '0.75rem', padding: '4px 10px', background: 'var(--theme-bg-secondary)', border: '1px solid var(--theme-border)', borderRadius: '6px', color: 'var(--theme-text-sub)', fontWeight: 600 }}>{t}</span>
              ))}
            </span>
          </div>
          <div>
            <div style={{ display: 'inline-flex', padding: '8px', borderRadius: '8px', background: 'rgba(16, 185, 129, 0.1)', color: '#10B981', marginBottom: '12px' }}>
              <Smartphone size={20} />
            </div>
            <h3 style={{ fontSize: '1.6rem', fontWeight: 800, color: 'var(--theme-text-main)', marginBottom: '12px' }}>Full Mobile Capability</h3>
            <p style={{ color: 'var(--theme-text-sub)', lineHeight: 1.6, fontSize: '0.95rem', marginBottom: '18px' }}>
